...
def main():
    ...
    # 사다리꼴 프로파일 생성
    t, theta_true, omega_true, alpha_true = generate_trapezoid_profile(dt, total_time, rpm_max=1000, accel_time=0.4)
    ...
    kf_cv.Q = 1e-4 * np.array([[dt**3/3, dt**2/2], [dt**2/2, dt]])
    kf_cv.R = np.array([[noise_std**2]])
    kf_cv.P = np.diag([1.0, 1.0])
    ...
    # 언랩된 상태로 추적
    theta_unwrap = theta_meas[0]
    
    for i in range(len(t)):
        if i > 0:
            kf_cv.predict()
            # 혁신(innovation) 계산 시 wrap 고려
            z = theta_meas[i]
            z_prev = theta_meas[i-1]
            # 측정값의 차이 (wrap 고려)
            dz = diffpi(z, z_prev)
            # 언랩 측정값
            theta_unwrap += dz
            ...
            # 언랩 상태로 업데이트
            kf_cv.update(np.array([[theta_unwrap]]))
        else:
            theta_unwrap = theta_meas[0]
            kf_cv.x[0, 0] = theta_unwrap
        
        theta_cv[i] = wrap_2pi(kf_cv.x[0, 0])
    ...
    axes[0].plot(t, omega_cv * 60/(2*np.pi), 'b-', label='CV Kalman', alpha=0.8)
    ...
    axes[2].plot(t, np.degrees(theta_cv), 'b-', label='CV Kalman', alpha=0.8)
    ...
    axes[3].plot(t, np.degrees(err_cv), 'b-', label=f'CV (RMS={rms_cv:.3f}°)', linewidth=1)
    ...
    print(f"CV 칼만 RMS: {rms_cv:.3f}°")
    print(f"개선율: {rms_meas/rms_cv:.2f}x")
    ...
    print(f"\n구간별 성능:")
    print(f"가속 구간: {np.degrees(np.sqrt(np.mean(err_cv[accel_idx]**2))):.3f}°")
    print(f"등속 구간: {np.degrees(np.sqrt(np.mean(err_cv[const_idx]**2))):.3f}°")
    print(f"감속 구간: {np.degrees(np.sqrt(np.mean(err_cv[decel_idx]**2))):.3f}°")
